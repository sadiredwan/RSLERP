...
{
    [Table("InvReturnTypes")]
    public class InvReturnType : IBaseModel
    {
        [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
        [Key]
        public int id { get; set; }
        [Required]
        [InvReturnTypeNameValidation(ErrorMessage = "Return type already exists.")]
        ...
        public class InvReturnTypeNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                ...
                try
                {
                    ...
                    var findInvReturnType = new DBContext().InvReturnTypes.Where(x => x.CompanyId == COMPANY_ID).ToList().FindAll(x => x.name == val);
                    if (findInvReturnType.Count > 0)
                    {

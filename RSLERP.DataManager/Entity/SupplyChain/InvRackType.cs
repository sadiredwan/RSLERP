...
{
    [Table("InvRackTypes")]
    public class InvRackType : IBaseModel
    {
        [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
        [Key]
        public int id { get; set; }
        [Required]
        [InvRackTypeNameValidation(ErrorMessage = "Rack type already exists.")]
        ...
        public class InvRackTypeNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                ...
                try
                {
                    ...
                    var findInvRackType = new DBContext().InvRackTypes.Where(x => x.CompanyId == COMPANY_ID).ToList().FindAll(x => x.name == val);
                    if (findInvRackType.Count > 0)
                    {

...
{
    [Table("InvDamageTypes")]
    public class InvDamageType : IBaseModel
    {
        [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
        [Key]
        public int id { get; set; }
        [Required]
        [InvDamageTypeNameValidation(ErrorMessage = "Damage type alredy exists.")]
        ...
        public class InvDamageTypeNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                ...
                try
                {
                    ...
                    var findInvDamageType = new DBContext().InvDamageTypes.Where(x => x.CompanyId == COMPANY_ID).ToList().FindAll(x => x.name == val);
                    if (findInvDamageType.Count > 0)
                    {

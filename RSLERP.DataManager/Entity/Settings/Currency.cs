...
namespace RSLERP.DataManager.Entity
{
    ...
    public class Currency : IBaseModel
    {
        ...
        public class CurrencyNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                ...
                try
                {
                    ...
                    var findCurrencyName = new DBContext().Currencies.ToList().FindAll(x => x.name == val);
                    if (findCurrencyName.Count > 0)
                    {
        ...
        [AttributeUsage(AttributeTargets.Property | AttributeTargets.Field, AllowMultiple = false)]
        public class CurrencyShortNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                if (!(value is string)) return new ValidationResult("DataType Error");
                ...
                try
                {
                    ...
                    var findCurrencyShortName = new DBContext().Currencies.ToList().FindAll(x => x.short_name == val);
                    if (findCurrencyShortName.Count > 0)
                    {
                        return new ValidationResult(FormatErrorMessage(validationContext.MemberName));
                    }
                }

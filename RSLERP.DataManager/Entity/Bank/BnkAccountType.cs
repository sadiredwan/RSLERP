...
{
    [Table("BnkAccountTypes")]
    public class BnkAccountType : IBaseModel
    {
        ...
        [Required]
        public int pay_or_receive { get; set; }
        ...
        public class BnkAccountTypeNameValidation : ValidationAttribute
        {
            protected override ValidationResult IsValid(object value, ValidationContext validationContext)
            {
                ...
                try
                {
                    ...
                    var findBnkAccountType = new DBContext().BnkAccountTypes.Where(x => x.CompanyId == COMPANY_ID).ToList().FindAll(x => x.name == val);
                    if (findBnkAccountType.Count > 0)
                    {
